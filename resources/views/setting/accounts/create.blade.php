...
<body class="bg-gray-100">
  <div class="mx-auto mt-10 max-w-md rounded-lg bg-white p-4 shadow-md">
    ...
    <h1 class="mb-6 text-center text-xl font-bold">アカウント作成</h1>
    ...
    <form class="flex flex-col gap-4" action="{{ route('settings.accounts.store') }}" method="POST">
      @csrf
      <div>
        ...
        <input class="w-full rounded border border-gray-300 px-3 py-2 focus:border-blue-500 focus:outline-none"
          id="login_id" name="login_id" type="text" value="{{ old('login_id') }}" required>
        @error('login_id')
      ...
      <div>
        ...
        <div class="flex items-center">
          <label class="ms-4">
            <input name="role" type="radio" value="管理者" {{ old('role') === '管理者' ? 'checked' : '' }} required>
            管理者
          </label>
          <label class="ms-4">
            <input name="role" type="radio" value="一般" {{ old('role') === '一般' ? 'checked' : '' }} required>
            一般
          </label>
        </div>
        @error('role')
      ...
      <div class="mb-4">
        <label class="block text-sm font-medium text-gray-700" for="password-confirmation">新しいパスワード確認</label>
        <input class="mt-1 block w-full rounded-md border border-gray-300 p-2" id="password-confirmation"
          name="password_confirmation" type="password" required>
      </div>
      <div>
        <button class="w-full rounded-md bg-blue-500 py-2 font-bold text-white hover:bg-blue-600" type="submit">
          作成
        </button>
      </div>
    </form>
